...
def calc_ranks(idx, label, pred_score):
    ...
    b_range = torch.arange(pred_score.size()[0]) #[0,1,2...size-1]
    target_pred = pred_score[b_range, idx] #正确位置数据备份
    pred_score = torch.where(label.bool(), -torch.ones_like(pred_score) * 10000000, pred_score) #将所有正确位置分数赋值为负无穷避免影响
    pred_score[b_range, idx] = target_pred #将正确位置数据备份回来
